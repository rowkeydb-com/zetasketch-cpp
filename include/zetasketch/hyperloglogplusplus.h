...
class HyperLogLogPlusPlus {
 public:
  ...
  // Walks a sparse sketch's stored stream and reports the first defect
  // in it, or nothing when there is none. FromBytes checks what the
  // reference checks when it reads, which stops short of the stream's
  // contents; those are read lazily, by whichever later operation first
  // needs them, and a defect is reported then, from a sketch that may
  // by that time have been stored. This is the check to make at the
  // point bytes arrive from a caller that is not trusted, so that a
  // sketch whose stream is broken is refused there instead.
  //
  // Refused: a sparse stream that does not decode, does not strictly
  // increase, or holds a number of values other than the recorded
  // sparse size. Nothing the reference writes is refused.
  //
  // Not refused: anything about the values themselves. The reference
  // writes sparse values out of range for the recorded precision, two
  // values naming one index, and dense registers above the largest a
  // hash can produce, whenever it merges a lower-precision operand into
  // a sketch holding unflushed values, which it carries across as they
  // are; this library writes the same bytes. An operation that reads
  // such a value refuses it then, as an error value, and nothing faults
  // on it. A dense sketch's sparse fields are not read here, because
  // neither library reads them anywhere.
  //
