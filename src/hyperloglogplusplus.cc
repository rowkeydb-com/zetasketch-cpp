...
// Decodes the whole of a sparse stream and checks what every stream the
// reference writes satisfies: each value increases on the one before
// it, and there are as many values as the recorded sparse size says.
// The encoder refuses a value that does not increase, and the flush
// records the count it wrote.
//
// Nothing else about the values is checked, because nothing else holds
// of the reference's own output. Its downgrade re-encodes the stored
// stream for the lower precision but carries the unflushed buffer
// across as it is, so a sketch it writes after merging a
// lower-precision operand into a receiver holding unflushed values can
// carry values encoded at the old sparse precision: out of range for
// the one recorded, naming an index another value names, or holding a
// rho that on promotion exceeds the largest a hash can produce. This
// library writes the same bytes, as it must. Such a value is refused
// by whichever operation reads it, as an error value; nothing faults.
std::expected<void, utils::Error> ValidateSparseStream(
    const hll::State& state) {
  size_t count = 0;
  ...
  if (std::cmp_not_equal(count, state.sparse_size)) {
    return std::unexpected(utils::Error{
        .code = utils::ErrorCode::kInvalidState,
...
std::expected<HyperLogLogPlusPlus, utils::Error> HyperLogLogPlusPlus::Create(
  ...
  // refuse to parse back: the types that admit no kind of addition.
  if (AdmittedKinds(value_type) == 0) {
    return std::unexpected(utils::Error{
        .code = utils::ErrorCode::kIllegalArgument,
        .message =
...
std::expected<HyperLogLogPlusPlus, utils::Error> HyperLogLogPlusPlus::Create(
  ...
  // top of its own type, which the precision check refuses anyway,
  // cannot overflow on the way there.
...
std::expected<void, utils::Error> HyperLogLogPlusPlus::Validate() const {
  ...
  // that fails them. What remains unread until here is a sparse
  // sketch's stream. A dense sketch's registers are not bounded: the
  // reference writes registers above the largest a hash can produce
  // when it promotes values carried across a downgrade, and both
  // libraries estimate any register value without faulting. A dense
  ...
  if (const auto* sparse =
    ...
    return ValidateSparseStream(sparse->state());
  }
  return {};
}
