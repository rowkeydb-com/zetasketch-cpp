...
TEST(ErrorHandlingTest, CreateWithoutASparsePrecisionUsesTheBuilderDefault) {
  ...
  // from it.
  // The extremes of the type as well: the reference adds five to the
  // normal precision in its own width and wraps, this library adds in
  // a wider one, and both refuse with the same words either way.
...
// A dense sketch's registers are not bounded by the walk. The reference
// writes registers above the largest a hash can produce when it
// promotes values carried across a downgrade, so a bound would refuse
// its output; and every register value estimates without faulting, in
// both libraries, so there is nothing to protect against.
TEST(ErrorHandlingTest, ValidateAcceptsAnyRegisterValue) {
  for (const uint8_t register_value :
       {uint8_t{61}, uint8_t{62}, uint8_t{0x80}, uint8_t{0xFF}}) {
    State state;
    state.encoding_version = 2;
    state.precision = 4;
    state.sparse_precision = 0;
    state.data = std::vector<uint8_t>(16, 0);
    (*state.data)[3] = register_value;
    auto bytes = state.ToByteArray();
    ASSERT_TRUE(bytes.has_value());
    auto sketch = HyperLogLogPlusPlus::FromBytes(*bytes);
    ASSERT_TRUE(sketch.has_value());
    EXPECT_TRUE(sketch->Validate().has_value()) << register_value;
    EXPECT_TRUE(sketch->Result().has_value()) << register_value;
  }
}
...
// lower precision but carries its unflushed values across as they are,
// so a sketch it writes after merging a lower-precision operand holds
// values encoded at its old sparse precision. Both libraries write the
// same bytes, and the full walk must pass them. The bytes are the
// reference's own for the same operations.
//
// Where such a sketch is then promoted under the lower precision, the
// reference throws "Index 19 out of bounds for length 16" for a
// receiver at (4, 6) holding three values given a (4, 4) operand and
// three more values; this library returns an error value for the same
// sequence, and the sketch written before the additions is the
// reference's byte for byte.
TEST(ErrorHandlingTest, ValidateAcceptsValuesCarriedAcrossADowngrade) {
  struct Downgrade {
    int32_t receiver_sparse_precision;
    int32_t operand_sparse_precision;
    const char* reference_hex;
  };
  const std::vector<Downgrade> downgrades = {
      {7, 4, "087010061802200b82071110061804200432092c2d189107ff01c004"},
      {25, 7,
       "087010061802200b82071a10061807200732128241bf12c023dbc9c805f8f1d005a0a6"
       "f602"},
  };
  for (const Downgrade& downgrade : downgrades) {
    const int32_t precision = downgrade.operand_sparse_precision;
    auto receiver = HyperLogLogPlusPlus::Create(
        precision, downgrade.receiver_sparse_precision);
    auto operand = HyperLogLogPlusPlus::Create(
        precision, downgrade.operand_sparse_precision);
    ASSERT_TRUE(receiver.has_value() && operand.has_value());
    for (const std::string_view value : {"a0", "a1", "a2"}) {
      ASSERT_TRUE(receiver->Add(value).has_value());
    }
    for (const std::string_view value : {"b0", "b1", "b2"}) {
      ASSERT_TRUE(operand->Add(value).has_value());
    }
    ASSERT_TRUE(receiver->Merge(std::move(*operand)).has_value());
    ...
    EXPECT_EQ(SerializedHex(*receiver), downgrade.reference_hex);

    auto reread =
        HyperLogLogPlusPlus::FromBytes(DecodeHex(downgrade.reference_hex));
    ASSERT_TRUE(reread.has_value());
    valid = reread->Validate();
    EXPECT_TRUE(valid.has_value())
        << (valid.has_value() ? "" : valid.error().message);
  }

  // The sequence the reference throws on, and the write before it that
  // both libraries make identically.
  auto receiver = HyperLogLogPlusPlus::Create(4, 6);
  auto operand = HyperLogLogPlusPlus::Create(4, 4);
  ASSERT_TRUE(receiver.has_value() && operand.has_value());
  for (const std::string_view value : {"r0", "r1", "r2"}) {
    ASSERT_TRUE(receiver->Add(value).has_value());
  }
  for (const std::string_view value : {"o0", "o1", "o2"}) {
    ASSERT_TRUE(operand->Add(value).has_value());
  }
  ASSERT_TRUE(receiver->Merge(std::move(*operand)).has_value());
  EXPECT_EQ(SerializedHex(*receiver),
            "087010061802200b82070f100518042004320713070f9b0bfd03");
  EXPECT_EQ(receiver->Result().value_or(-1), 6);
  for (const std::string_view value : {"x", "y", "z"}) {
    ASSERT_TRUE(receiver->Add(value).has_value());
  }
  auto refused = receiver->Serialize();
  ASSERT_FALSE(refused.has_value());
  EXPECT_EQ(refused.error().code, zetasketch::utils::ErrorCode::kInvalidState);
}

// A set narrowed by one merge is the input to the next. The refusals
// and the bytes are the reference's own.
TEST(ErrorHandlingTest, ChainedMergesNarrowTheAdmittedKindsAsTheReferenceDoes) {
  using zetasketch::hll::ValueType;

  // Untyped, then text, then longs: the second merge is refused, and
  // the text addition afterwards narrows a two-kind set and records
  // its type.
  {
    auto receiver =
        HyperLogLogPlusPlus::FromBytes(DecodeHex(kUntypedAt10And15));
    auto text =
        HyperLogLogPlusPlus::Create(10, 15, ValueType::kBytesOrUtf8String);
    auto longs = HyperLogLogPlusPlus::Create(10, 15, ValueType::kUnsignedInt64);
    ASSERT_TRUE(receiver.has_value() && text.has_value() && longs.has_value());
    ASSERT_TRUE(receiver->Merge(std::move(*text)).has_value());
    auto refused = receiver->Merge(std::move(*longs));
    ASSERT_FALSE(refused.has_value());
    EXPECT_EQ(refused.error().message,
              "Aggregator of type [STRING, BYTES] is incompatible with "
              "aggregator of type [LONG]");
    ASSERT_TRUE(receiver->Add("a").has_value());
    EXPECT_EQ(SerializedHex(*receiver),
              "087010011802200b82070b1001180a200f3203bc9401");
  }

  // Untyped, then longs, then untyped: the set stays at longs alone
  // through the second merge, and the long added afterwards records
  // nothing.
  {
    auto receiver =
        HyperLogLogPlusPlus::FromBytes(DecodeHex(kUntypedAt10And15));
    auto longs = HyperLogLogPlusPlus::Create(10, 15, ValueType::kUnsignedInt64);
    auto untyped = HyperLogLogPlusPlus::FromBytes(DecodeHex(kUntypedAt10And15));
    ASSERT_TRUE(receiver.has_value() && longs.has_value() &&
                untyped.has_value());
    ASSERT_TRUE(receiver->Merge(std::move(*longs)).has_value());
    ASSERT_TRUE(receiver->Merge(std::move(*untyped)).has_value());
    auto refused = receiver->Add("a");
    ASSERT_FALSE(refused.has_value());
    EXPECT_EQ(refused.error().message,
              "unable to add type STRING to aggregator of type [LONG]");
    ASSERT_TRUE(receiver->Add(int64_t{5}).has_value());
    EXPECT_EQ(SerializedHex(*receiver),
              "08701001180282070a1001180a200f3202df1f");
  }
}
